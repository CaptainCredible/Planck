void CC(int plexer, int rawVal, int current) {
  ...
  if (rawDiff <= -8 || rawDiff >= 8) {                                                                    //only do anything if the raw value that we read has changed by more than 8
    ...
    if (oldMIDIVal[current] != data_send) {                                                //has the MIDI value changed?
      if (lfoOnState && SHIFT && current < 4) {                                            //is the LFO on? do we want to ignore LFO knobs? HEY THIS WONT WORK IF WE REMAP THE CC!
        lfoControls[current] = rawVal;                                                     //harvest the Raw Values for use as higher rex LFO controls
